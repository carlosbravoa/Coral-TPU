...
***This example is intended to run later in a raspberry PI, but for now, is running on a Lunux machine
***The only pending thing to make it run on the raspberry, since capturing frames require a different method
***through the picamera python library
...
def main():
  ...
  parser.add_argument(
      '--mode', help='Mode for de detection: OBJECT_DETECTION or IMAGE_CLASSIFICATION', required=True)
  parser.add_argument(
  ...
  while True:
        ...
        #TODO:
        #we are transforming the npimage to img, and the TPU library/utils are doing the inverse process
        #cv2_im = cv2.cvtColor(cv2_im,cv2.COLOR_BGR2RGB)  #The CV2 Way
        #pil_im = Image.fromarray(cv2_im)
        pil_im = Image.fromarray(np.uint8(cv2_im)).convert('RGB') #This is the tf utils way for the transformation. It needs numpy
...
def draw_rectangles(rectangles, image_np, label = None):
  ...
  if label:
    cv2.rectangle(image_np, (p1[0], p1[1]-20), (p2[0], p1[1]+10) ,color=(255, 0, 0), thickness = -1)
